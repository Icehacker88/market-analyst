...
报告同时展示测试集方向准确率、多数类基准、方向优势和滚动验证方向准确率。多数类基准用于识别“市场本来多数时间上涨”造成的虚高准确率；方向优势为负时，说明模型没有胜过简单多数类判断。只有滚动验证和独立测试都形成正方向优势时，日报才会给出 Up/Down 行动信号，否则显示 Observe，并保留原始预测用于研究。
